...
public abstract class Chart<T extends ChartData<? extends DataSet<? extends Entry>>> extends ViewGroup implements ChartInterface {
	...
	/**
	 * Deceleration friction coefficient in [0 ; 1] interval, higher values indicate that speed will decrease slowly, for example if it set to 0, it will stop
	 * immediately. 1 is an invalid value, and will be converted to 0.999f automatically.
	 */
	...
	protected boolean mForcedTouchEvent = false;
	...
	private String mNoDataText = "loading...";
	...
	private int[] mMarkerLineColors = new int[1];
	...
	/**
	 * Bitmap object used for drawing. This is necessary because hardware acceleration uses OpenGL which only allows a specific texture size to be drawn on the
	 * canvas directly.
	 **/
	...
	/**
	 * Returns true if there are values to highlight, false if there are no values to highlight. Checks if the highlight array is null, has a length of zero or
	...
	/**
	 * Highlights the values at the given indices in the given DataSets. Provide null or an empty array to undo all highlighting. This should be used to
	 * programmatically highlight values. This DOES NOT generate a callback to the OnChartValueSelectedListener.
	 *
	 * @param highs
	 */
	...
	/**
	 * Set a new (e.g. custom) ChartTouchListener NOTE: make sure to setTouchEnabled(true); if you need touch gestures on the chart
	 *
	 * @param l
	 */
	 ...
	 */
	protected boolean isDrawLastIndicator = true;

	@Override
	public boolean getDrawLastIndicator() {
		return this.isDrawLastIndicator;
	}

	protected void drawMarkers(Canvas canvas) {

		// if there is no marker view or drawing marker is disabled
		if (isSetSpecialMarkerView || mMarkerView == null || !mDrawMarkerViews || !valuesToHighlight())
			return;
	...
	protected void drawSpecialMarkers(Canvas canvas) {

		// System.out.println("mDrawMarkerViews:"+mDrawMarkerViews+"valuesToHighlight()"+valuesToHighlight());
		// if(true){
		// return ;
		// }
		// if there is no marker view or drawing marker is disabled
		...
		if (!isSetSpecialMarkerView || !mDrawMarkerViews || !valuesToHighlight())
			return;
		...
		for (int i = 0; i < mIndicesToHightlight.length; i++) {
			...
			if (xIndex <= mDeltaX && xIndex <= mDeltaX * mAnimator.getPhaseX()) {
				...
				isDrawLastIndicator = false;

				
				int markerIndex = mIndicesToHightlight[i].getDataSetIndex();
				List<String> xVals = mData.getXVals();

				mMarkerView.refreshContent(e, highlight, xVals.get(e.getXIndex()));
				mMarkerView.refreshContent(mIndicesToHightlight[i].getXIndex(), markerIndex, mData.getDataSets());

				mMarkerView.measure(MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED), MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED));
				mMarkerView.layout(0, 0, mMarkerView.getMeasuredWidth(), mMarkerView.getMeasuredHeight());
				mMarkerView.draw(canvas, pos[0], mMarkerView.getHeight() + Utils.convertDpToPixel(15f));
				// mMarkerView.draw(canvas, pos[0], 0);

				Paint paint = new Paint();
				paint.setColor(getSpecialMarkerLineColor()[markerIndex]);
				paint.setStrokeWidth(Utils.convertDpToPixel(2.5f));
				// Log.i("chart_info", "marker图表最大值:"+getYChartMax()+",marker图表最小值:"+getYChartMin());

				float[] pts = new float[] { pos[0], getYChartMax(), pos[0], getYChartMin() };

				applyTransformer(pts, markerIndex);
				
				// Log.i("chart_info", "marker图表最大值----:"+pts[1]+",marker图表最小值----:"+pts[3]);
				canvas.drawLine(pos[0], mMarkerView.getHeight() + Utils.convertDpToPixel(15f), pos[0], pts[3], paint);
				Bitmap specialMarkerCircle = getSpecialMarkerCircle()[markerIndex];
				float markerHeight = specialMarkerCircle.getHeight();
				float markerWidth = specialMarkerCircle.getWidth();

				canvas.drawBitmap(specialMarkerCircle, pos[0] - markerWidth / 2, pos[1] - markerHeight / 2, paint);
			}
		}
	}
	
	public void setSpecialMarkerLineColor(int[] colors){
		this.mMarkerLineColors = colors;
	}
	
	public void setSpecialMarkerLineColor(int color){
		mMarkerLineColors[0] = color;
	}
	
	public int[] getSpecialMarkerLineColor(){
		return mMarkerLineColors;
	}
	...
	/**
	 * Animates the drawing / rendering of the chart on both x- and y-axis with the specified animation time. If animate(...) is called, no further calling of
	 * invalidate() is necessary to refresh the chart. ANIMATIONS ONLY WORK FOR API LEVEL 11 (Android 3.0.x) AND HIGHER.
	 *
	 ...
	 * necessary to refresh the chart. ANIMATIONS ONLY WORK FOR API LEVEL 11 (Android 3.0.x) AND HIGHER.
	 *
	...
	/**
	 * If set to true, value highlighting is enabled for all underlying data of the chart which means that all values can be highlighted programmatically or by
	 * touch gesture.
	 *
	 * @param enabled
	 */
	...
	public void setForceTouchEventEnable(boolean enabled) {
		this.mForcedTouchEvent = enabled;
	}
	 ...
	 * @description TODO()
	 * @author liuchen
	 * @param isSetSpecialMarkerView
	 */
	protected boolean isSetSpecialMarkerView = false;
	protected Bitmap[] markerCircle  = new Bitmap[1];
	protected boolean highlightAutoDismiss = false;

	public void setSpecialMarkerView(boolean isSetSpecialMarkerView) {
		this.isSetSpecialMarkerView = isSetSpecialMarkerView;
	}

	public boolean isSetSpecialMarkerView() {
		return this.isSetSpecialMarkerView;
	}

	public void setSpecialMarkerCircle(Bitmap circleBitmap) {
		this.markerCircle[0] = circleBitmap;
	}
	
	public void setSpecialMarkerCircle(Bitmap[] circleBitmap) {
		this.markerCircle = circleBitmap;
	}

	public Bitmap[] getSpecialMarkerCircle() {
		return this.markerCircle;
	}

	public boolean isHighlightAutoDismiss() {
		return highlightAutoDismiss;
	}

	public void setHighlightAutoDismiss(boolean highlightAutoDismiss) {
		this.highlightAutoDismiss = highlightAutoDismiss;
	}

	/**
	 * Returns the Legend object of the chart. This method can be used to get an instance of the legend in order to customize the automatically generated
	 * Legend.
	 *
	 * @return
	 */
	...
	/**
	 * Set this to true to draw a user specified marker-view when tapping on chart values (use the setMarkerView(MarkerView mv) method to specify a marker
	 * view). Default: true
	 *
	 * @param enabled
	 */
	...
	/**
	 * Get all Entry objects at the given index across all DataSets. INFORMATION: This method does calculations at runtime. Do not over-use in performance
	 * critical situations.
	 *
	 * @param xIndex
	 * @return
	 ...
	 * card chart is saved as a PNG image, example: saveToPath("myfilename", "foldername1/foldername2");
	 *
	 * @param title
	 * @param pathOnSD
	...
	/**
	 * Saves the current state of the chart to the gallery as a JPEG image. The filename and compression can be set. 0 == maximum compression, 100 = low
